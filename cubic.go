...
func NewCubic() *Cubic {
	return &Cubic{
		...
		loc:  univariate.NewLocation(),
		grad: univariate.NewGradient(),
...
func (c *Cubic) SetDisp(b bool) {
	c.disp = b
}

func (c *Cubic) Loc() *univariate.Location {
	return c.loc
}

func (c *Cubic) Obj() *univariate.Objective {
	return c.obj
}

func (c *Cubic) Grad() *univariate.Gradient {
	return c.grad
}

func (c *Cubic) Step() *univariate.BoundedStep {
	return c.step
}

func (c *Cubic) Fun() optimize.SisoGrad {
	return c.fun
}
...
func (c *Cubic) Initialize() (err error) {
	...
	initLoc := c.loc.Init()
	if math.IsNaN(initLoc) {
		return errors.New("gofunopter: cubic: initial location is NaN")
	}

	initObj := c.obj.Init()
	initGrad := c.grad.Init()
	...
	if math.IsNaN(initObj) {
		...
		initObj, initGrad, err := c.fun.Eval(initLoc)
		if err != nil {
			return errors.New("gofunopter: cubic: error calling function during optimization")
		}
		c.obj.SetInit(initObj)
		c.grad.SetInit(initGrad)
	} else {
		if math.IsNaN(initGrad) {
			return errors.New("gofunopter: cubic: initial function value and gradient must either both be set or neither set")
		}
	...
	err = optimize.Initialize(c.loc, c.obj, c.grad, c.step)
	if err != nil {
...
func (c *Cubic) Converged() convergence.C {
	return convergence.CheckConvergence(c.obj, c.grad, c.step)
}

func (c *Cubic) AddToDisplay(d []*display.Struct) []*display.Struct {
	if c.disp {
		d = display.AddToDisplay(d, c.loc, c.obj, c.grad, c.step)
	}
	return d
}

func (c *Cubic) SetResult() {
	optimize.SetResult(c.loc, c.obj, c.grad, c.step)
}

func (cubic *Cubic) Iterate() (nFunEvals int, err error) {

	/* Break this into smaller functions so they are digestible
	 */
	...
	cubic.obj.AddToHist(trialF)
	cubic.grad.AddToHist(trialG)
